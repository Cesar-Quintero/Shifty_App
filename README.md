...
My focus was to construct seemless authentication, implment MVC structure for scalability, create a real-time calendar for schedule page, apply beautiful aesthetics to each page and allow for responsive designs and accessibility for my target audience.

## Optimizations

My next steps for improving this project would be to add transition the "request day off" data from local storage to the database like everything else. Add Google Auth for users who prefer to login with google credentials, transition from EJS to React for Views files for component-based frontend structure.
